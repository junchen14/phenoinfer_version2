...
from graphsage.random_rank_model import Rank_model
import torch
import random
...
def generate_label(positive_g,negative_genes):
    ...
    for gene in negative_genes:
        # label_select=random.choices(label_choice)[0]
        ...
        #
        #     positive_gene.append(positive_g)
        #     negati:ve_gene.append(gene)
        # else:
        #
        #     negative_gene.append(gene)
        #     positive_gene.append(positive_g)

    return positive_gene,negative_gene,label
...
def generate_embedding(embed_dic,entities):
    ...
    for entity in entities:
        ...
        #
        # embed_list.append(embeddings[id])
...
def generate_train_data(embed_dic,disease_gene,gene_set,disease_set):
    ...
    for disease in disease_set:
        ...
        for gene in genes:
            ...
            positive_gene = generate_embedding(embed_dic, positive_gene)

            negative_gene = generate_embedding(embed_dic, negative_gene)
            disease_embedding = generate_embedding(embed_dic, [disease] * negative_number)
...
def load_data(embed_dic,disease_gene,gene_set):
    ...
    g1,d,g2,y=generate_train_data(embed_dic,disease_gene,gene_set,train_diseases)




    return g1,d,g2,y,test_diseases,train_diseases


def train(model_,opt_,criterion_):
    model_.train()
    loader = TensorDataset(g1, g2, d, y)
    loader_dataset = DataLoader(loader, batch_size=125, shuffle=True)
    losses = 0

    for gene1_, gene2_, disease_, y_label in loader_dataset:
        gene1_batch = Variable(gene1_)
        gene2_batch = Variable(gene2_)
        disease_batch = Variable(disease_)

        predict_y = model_(gene1_batch, gene2_batch, disease_batch)
...
def ranked_auc(rank_list):
    ...
    for data in rank_dic.values():
        if (prior == 10000):
            ...
            prior = data
    with open("../calculate_auc/normal_rank.pkl","wb") as f:
        pkl.dump(rank_dic,f)
    return auc
...
if __name__ == '__main__':
    ...
    #
    #parser.add_argument("types",help="the type of the data")
    # args=parser.parse_args()
    ...
    data_type="union"
    # types=args.types
    #data_dir="/home/chenj0g/GraphSage/unsup-small_ngr/"+types+"_small_0.000010"
    ...
    g1,d,g2,y,test_disease,train_disease=load_data(dic,disease_genes,gene_list)
    ...
    for epoch in range(epoches):
        g1, d, g2, y = generate_train_data(dic, disease_genes, gene_list, train_disease)
        if epoch % 20 == 5:
            auc, rank = evaluation(model, test_disease, dic, gene_list)
            calculate_auc(auc)
            if rank < performance:
                torch.save(model, "../model/" + "model__" + str(rank) + ".pt")
