...
class EdgeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx,
        ...
        if context_pairs is None:
            ...
            print("loading the context pairs")
            edges = context_pairs
        ...
        print(len([n for n in G.nodes() if not G.node[n]['val'] and not G.node[n]['val']]), 'train nodes')
        print(len([n for n in G.nodes() if G.node[n]['val'] or G.node[n]['val']]), 'test nodes')
    ...
    def _remove_isolated(self, edge_list):
        ...
        for n1, n2 in edge_list:
            ...
            if (self.deg[self.id2idx[n1]] == 0 or self.deg[self.id2idx[n2]] == 0):
                # and (not self.G.node[n1]['val'] or self.G.node[n1]['val']) \
                # and (not self.G.node[n2]['val'] or self.G.node[n2]['val']):
                continue
            else:
    ...
    def construct_adj(self):
        ...
        for nodeid in self.G.nodes():
            if self.G.node[nodeid]['val'] or self.G.node[nodeid]['val']:
                continue
    ...
    def label_val(self):
        ...
        for n1, n2 in self.G.edges():
            if (self.G.node[n1]['val'] or self.G.node[n1]['val']
                    or self.G.node[n2]['val'] or self.G.node[n2]['val']):
