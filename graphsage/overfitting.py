...
def ranked_auc(rank_list):
    ...
    for data in rank_dic.values():
        if (prior == 10000):
            ...
            prior = data
    with open("../calculate_auc/inner_product.pkl","wb") as f:
        pkl.dump(rank_dic,f)
    return auc
...
if __name__ == '__main__':
    ...
    # test_disease=[disease for disease in test_disease]
    # random.shuffle(test_disease)


    auc,meanrank=evaluation(model,test_disease,dic,gene_list)
    print(auc)
    print("-----------------")
    calculate_auc(auc)
