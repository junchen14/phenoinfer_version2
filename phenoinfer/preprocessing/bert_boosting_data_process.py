...
type=["union"]
output_directory="../data/bert_traindata_only/"
...
for tp in type:
    ...
    model=gensim.models.Word2Vec.load("../opamodel/"+tp+"500.model")
    ...
    train_data = open(output_directory+tp+"_train500.txt", "w")
    train_data_group = open(output_directory+tp + "_train_group500.txt", "w")
    ...
    gene_list=[gene for gene in gene_set if gene in gene_name_embedding.keys()]
    ...
    for disease in disease_gene.keys():
        ...
        if disease in train_disease:
            ...
            for gene in genes:
                ...
                gene_vec=model[gene]
                gene_vec=np.append(gene_vec,gene_name_embedding[gene],0)
